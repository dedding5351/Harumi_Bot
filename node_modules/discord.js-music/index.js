...
module.exports = function(client, options) {
	...
	function play(msg, suffix) {
		...
		if (msg.author.voiceChannel === null) return msg.reply(msg, wrap('You\'re not in a voice channel.'));

		// Make sure the suffix exists.
		if (!suffix) return msg.reply(msg, wrap('No video specified!'));
		...
		if (queue.length >= MAX_QUEUE_SIZE) {
			return msg.reply(msg, wrap('Maximum queue size reached!'));
		}

		// Get the video information.
		msg.reply(msg, wrap('Searching...')).then(response => {
			...
			YoutubeDL.getInfo(suffix, ['-q', '--no-warnings', '--force-ipv4'], (err, info) => {
				...
				if (err || info.format_id === undefined || info.format_id.startsWith('0')) {
					return msg.reply(response, wrap('Invalid video!'));
				}

				// Queue the video.
				msg.reply(response, wrap('Queued: ' + info.title)).then(() => {
					queue.push(info);
	...
	function skip(msg, suffix) {
		...
		if (voiceConnection === null) return msg.reply(msg, wrap('No music being played.'));
		...
		msg.reply(msg, wrap('Skipped ' + toSkip + '!'));
	}
	...
	function queue(msg, suffix) {
		...
		msg.reply(msg, wrap('Queue (' + queueStatus + '):\n' + text));
	}
	...
	function pause(msg, suffix) {
		...
		// Pause.
		msg.reply(msg, wrap('Playback paused.'));
	...
	function resume(msg, suffix) {
		...
		// Resume.
		msg.reply(msg, wrap('Playback resumed.'));
		if (voiceConnection.playingIntent) voiceConnection.resume();
	}
	...
	function executeQueue(msg, queue) {
		...
		if (queue.length === 0) {
			msg.reply(msg, wrap('Playback finished.'));
		...
		new Promise((resolve, reject) => {
			...
			// Play the video.
			msg.reply(msg, wrap('Now Playing: ' + video.title)).then(() => {
